...
const whiteSkin = new Skin({ fill: GRAY })
const heartTexture = new Texture('heart.png')
const heartSkin = new Skin({
...
let AvatarContainer = Container.template($ => ({
  skin: avatarSkin,
  ...
  contents: [Content($, { skin: stripSkin, variant: 0 })]
}))
...
let StripContainer = Container.template($ => ({
  ...
  Behavior: class extends Behavior {
    ...
    onTimeChanged (container) {
      container.first.variant = 34 * container.fraction
    }
  },
  contents: [Content($, { skin: stripSkin, variant: 0 })]
}))
  ...
  Behavior: class extends Behavior {
    onCreate (application, anchors) {
      ...
      this.timeline = new Timeline()
        .to(anchors.strip, { variant: 17 }, 3000, null, 0)
        .to(anchors.strip, { variant: 17 }, 1000, null, 0)
        .to(anchors.strip, { variant: 17, state: 0, y: -92 }, 1000, null, 0)
        .to(anchors.heart, { state: 1, y: 72 }, 500, Math.quadEaseOut, -250)
        .to(anchors.logo, { y: 60 }, 500, Math.quadEaseOut, 500)
    ...
    onDisplaying (application) {
      if (application.width != 128 || application.height != 128) {
        trace(
          'WARNING: This application was designed to run on a 128x128 screen.\n'
        )
      }
    }
    ...
    onTimeChanged (application) {
      ...
      if (this.reverse) time = application.duration - time
      this.timeline.seekTo(time)
    }
  },
  contents: [
    Content($, { anchor: 'heart', skin: heartSkin, top: 240, state: 0 }),
    Content($, {
      anchor: 'strip',
      skin: stripSkin,
      top: 60,
    ...
    Content($, { anchor: 'logo', skin: logoSkin, top: 240 })
  ]
}))
