...
class SecureHealthThermometerServer extends BLEServer {
  onReady() {
    ...
    this.securityParameters = {
      mitm: false,
      ioCapability: IOCapability.NoInputOutput
    };
    this.onDisconnected();
    this.deploy();
  }
  onAuthenticated() {
    ...
    if (this.characteristic) this.startMeasurements();
  }
  onConnected() {
    this.connected = true;
  ...
  onDisconnected() {
    ...
    this.startAdvertising({
      advertisingData: {
        ...
        completeUUID16List: [uuid`1809`, uuid`180F`]
      }
    });
  }
  onCharacteristicNotifyEnabled(characteristic) {
    this.characteristic = characteristic;
    if (this.authenticated) this.startMeasurements();
  }
  ...
  get temperature() {
    if (98.5 > this.temp) this.temp += 0.1;
